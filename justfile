...
install *dropins:
    ...
    sudo rm -f "$BASE/00-base.conf" "$QD/$NAME@headless.container" "$QD/{{mybox_netfile}}"
    ...
    sudo tee "$BASE/00-network.conf" >/dev/null <<EOF
    ...
    if [[ "$GUI" == 1 ]]; then
      ...
      sed -e "s|^PathExists=.*|PathExists={{mybox_wayland}}|" \
          -e "s|^Unit=.*|Unit=$NAME@gui.service|" \
          "$SRC/host/mybox-gui.path" | sudo tee "/etc/systemd/system/$NAME-gui.path" >/dev/null
      sudo chmod 0644 "/etc/systemd/system/$NAME-gui.path"
      sudo systemctl daemon-reload
      sudo systemctl enable --now "$NAME-gui.path"
      echo "    instances: $NAME@headless (boot) + $NAME@gui ({{mybox_wayland}})"
    else
      if [[ -e "$QD/$NAME@gui.container" ]]; then
        ...
        sudo rm -f "/etc/systemd/system/$NAME-gui.path" "$QD/$NAME@gui.container"
        sudo rm -rf "$GUID"
      fi
...
start:
    ...
    if [[ -e "{{quadlet_dir}}/{{mybox_name}}@gui.container" ]]; then
      ...
      # the box would never swap again after a stop/start cycle.
      sudo systemctl start "{{mybox_name}}-gui.path"
      if [[ -e "{{mybox_wayland}}" ]]; then sudo systemctl start "{{gui}}"; exit 0; fi
    fi
